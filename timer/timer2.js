...
var depDateStrShort;
var depDateStrLong;
var depDateStrCustom;

var retDateStrShort;
var retDateStrLong;
var retDateStrCustom;
var selDate;
...
var fullMonArr = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];

function showCalendar(where, startDate){
   
    if(startDate != "" && startDate != null){
        var returnValues = getParsedDates(startDate);
        var date = startDate.substring(5, 50);
        var dateArr = date.split('-');
        var mon = (monArr.indexOf(dateArr[1])) + 1;
        if(mon < 10) mon = "0" + mon;
            endDate = createCustomDate(Calendar.intToDate(((parseInt(dateArr[2])+1).toString())+(mon.toString())+dateArr[0]));
    }else{
        var startYea = today.getFullYear().toString();
        var startMon = (today.getMonth()+1).toString();
        var startDat = today.getDate().toString();

        var endYea = (today.getFullYear()+1).toString();

        if(startMon.length < 2) startMon = "0" + startMon;
        if(startDat.length < 2) startDat = "0" + startDat;

        startDate = createCustomDate(Calendar.intToDate(startYea+startMon+startDat));
        endDate = createCustomDate(Calendar.intToDate(endYea+startMon+startDat));
...
function renderCal(){
    fareObj = getFareFor(12, 2012);
    Calendar.appendFares();
}

function getFareFor(month, year){
    var fareObj = {"20121201":5412, "20121202":2145, "20121203":1137, "20121210":3636};
    return fareObj;
}

function addListeners(){

        var _depDateStrShort;
        var _depDateStrLong;
        var _depDateStrCustom;

        var _retDateStrShort;
        var _retDateStrLong;
        var _retDateStrCustom;
        
        LEFT_CAL.addEventListener("onSelect", function(){

            _depDateStrShort  = (this.selection.print('%Y')+this.selection.print('%m')+this.selection.print('%d'));
            _depDateStrLong   = Calendar.intToDate(_depDateStrShort);
            _depDateStrCustom = createCustomDate(_depDateStrLong);

            _retDateStrShort  = ((parseInt(this.selection.print('%Y'))+1)+this.selection.print('%m')+this.selection.print('%d'));
            _retDateStrLong   = Calendar.intToDate(_retDateStrShort);
            _retDateStrCustom = createCustomDate(_retDateStrLong);

            var dateVariablesArr = [this.selection.print('%d'), this.selection.print('%m'), this.selection.print('%Y'), _depDateStrCustom, _depDateStrLong, _depDateStrShort, _retDateStrCustom, _retDateStrLong, _retDateStrShort];

            selDate = _depDateStrShort;
        });

        LEFT_CAL.addEventListener("onChange", function(){

        });
        
    }

    function createCustomDate(dateStrLong){
        var dateArr = String(dateStrLong).split(' ');
        return (dateArr[0]+", "+dateArr[2]+"-"+dateArr[1]+"-"+dateArr[3]);
        // Wed Sep 26 2012 12:00:00 GMT+0530 (India Standard Time)
    }

    function getParsedDates(date){
        date = date.substring(5, 50);
        var dateArr = date.split('-');
        var mon = (monArr.indexOf(dateArr[1])) + 1;
        if(mon < 10) mon = "0" + mon; 
        // set values that can be used later;
        depDateStrShort  = (dateArr[2]+mon+dateArr[0]);
        depDateStrLong   = Calendar.intToDate(depDateStrShort);
        depDateStrCustom = date;
        return [depDateStrLong, depDateStrShort];
    }
...
Calendar = function(){
    ...
    function $(a, b, c) {
        if (b instanceof Array) for (var d = b.length; --d >= 0;) $(a, b[d], c);
        else Y(b, c, a ? c : null);
        return a
    }
    function Z(a, b) {
    ...
    function W(a, b, c, d) {
        ...
        else if (typeof b == "object") for (var f in b) b.hasOwnProperty(f) && W(a, f, b[f], c);
    ...
    function V(a, b, c, d) {
        ...
        else if (typeof b == "object") for (var f in b) b.hasOwnProperty(f) && V(a, f, b[f], c);
    ...
    function appendFares(){

        var dateStr = currentDateObj.getDate().toString();
        var monthStr = (parseInt(currentDateObj.getMonth())+1).toString();
        var yearStr = currentDateObj.getFullYear().toString();
        
        for(var obj in fareObj){
            for(var i = 1; i <= 42; i++){
                i = i < 10 ? "0"+i : i;
                if((yearStr+monthStr+i).toString() == obj.toString()){
                    console.log("fareSpan: "+document.getElementById("fareSpan").parentNode.innerHTML);
                    // document.getElementById("fareSpan").parentNode.innerHTML = fareObj[obj];

                }else{
                    console.log("else");
                    // document.getElementById("fareSpan").parentNode.innerHTML = "<br>";
                }
            }
        }
    }
    // {"20121126":3079, "20121127":1424, "20121210":5412, "20121211":2145, "20121212":1137, "20121231":3636}
    ...
    var b = navigator.userAgent,
        ...
        h = a.prototype,
        i = a.I18N = {};
    a.SEL_NONE = 0, a.SEL_SINGLE = 1, a.SEL_MULTIPLE = 2, a.SEL_WEEK = 3, a.dateToInt = O, a.intToDate = P, a.appendFares = appendFares, a.printDate = R, a.formatString = K, a.i18n = L, a.LANG = function (a, b, c) {
        i.__ = i[a] = {
            name: b,
            data: c
        }
